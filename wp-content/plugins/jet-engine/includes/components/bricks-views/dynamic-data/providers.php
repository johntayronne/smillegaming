...
//		add_action( 'rest_api_init', [ $instance, 'register_tags' ], 8 );
	...
	public function get_tag_value( $tag, $post, $context = 'text' ) {
		...
		do_action('jet-engine/bricks-views/dynamic-data/before_render');

		$value = $this->providers[ $provider ]->get_tag_value( $tag, $post, $args, $context );

		do_action( 'jet-engine/bricks-views/dynamic-data/after_render' );
