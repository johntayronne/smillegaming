...
use ILKinguinVendor\WPDesk\Logger\WC\Exception\WCLoggerAlreadyCaptured;
/**
...
class WooCommerceCapture
{
    ...
    public function __construct(\ILKinguinVendor\Monolog\Logger $monolog)
    {
    ...
    private function prepareCaptureHookCallable()
    {
        ...
        if ($this->captureHookFunction === null) {
            $this->captureHookFunction = function () use($monolog) {
                return new \ILKinguinVendor\WPDesk\Logger\WC\WooCommerceMonologPlugin($monolog, $this->originalWCLogger);
            };
            $this->monolog->pushHandler(new \ILKinguinVendor\WPDesk\Logger\WC\WooCommerceHandler($this->originalWCLogger));
        }
    }
    /**
    ...
    public function captureWcLogger()
    {
        if (self::isSupportedWCVersion()) {
            if ($this->isCaptured) {
                throw new \ILKinguinVendor\WPDesk\Logger\WC\Exception\WCLoggerAlreadyCaptured('Try to free wc logger first.');
            }
