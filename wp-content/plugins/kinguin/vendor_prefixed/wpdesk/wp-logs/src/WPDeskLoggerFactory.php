...
use LogicException;
use ILKinguinVendor\Monolog\Handler\NullHandler;
use ILKinguinVendor\Monolog\Logger;
use ILKinguinVendor\Monolog\Registry;
use ILKinguinVendor\Monolog\Handler\StreamHandler;
use Psr\Log\LogLevel;
use ILKinguinVendor\WPDesk\Logger\WC\WooCommerceCapture;
use ILKinguinVendor\WPDesk\Logger\WP\WPCapture;
/**
 * Manages and facilitates creation of logger
 *
 * @package WPDesk\Logger
 */
class WPDeskLoggerFactory extends \ILKinguinVendor\WPDesk\Logger\BasicLoggerFactory
{
    ...
    public static function tearDown($name = self::DEFAULT_LOGGER_CHANNEL_NAME)
    {
        if (\ILKinguinVendor\Monolog\Registry::hasLogger($name)) {
            \ILKinguinVendor\Monolog\Registry::removeLogger($name);
        }
    }
    /**
    ...
    public function disableLog($name = self::DEFAULT_LOGGER_CHANNEL_NAME)
    {
        ...
        if (\ILKinguinVendor\Monolog\Registry::hasLogger($name)) {
            /** @var Logger $logger */
            $logger = \ILKinguinVendor\Monolog\Registry::getInstance($name);
    ...
    public function createWPDeskLogger($name = self::DEFAULT_LOGGER_CHANNEL_NAME)
    {
        if (!self::$shouldLoggerBeActivated) {
            return new \ILKinguinVendor\Monolog\Logger($name);
        }
        if (\ILKinguinVendor\Monolog\Registry::hasLogger($name)) {
    ...
    private function captureWPLog()
    {
        static $wpCapture;
        if (!$wpCapture) {
            $wpCapture = new \ILKinguinVendor\WPDesk\Logger\WP\WPCapture(\basename($this->getFileName()));
     ...
     * @param Logger $logger
     */
    private function captureWooCommerce(\ILKinguinVendor\Monolog\Logger $logger)
    {
        if (!\defined('ILKinguinVendor\\WC_LOG_THRESHOLD')) {
            \define('ILKinguinVendor\\WC_LOG_THRESHOLD', self::LEVEL_WC);
        }
        $wcIntegration = new \ILKinguinVendor\WPDesk\Logger\WC\WooCommerceCapture($logger);
     ...
     */
    private function pushFileHandle($filename, \ILKinguinVendor\Monolog\Logger $logger)
    {
        $logger->pushHandler(new \ILKinguinVendor\Monolog\Handler\StreamHandler($filename, self::LEVEL_WPDESK_FILE));
    }
    /**
    ...
    public function getFileName($name = self::DEFAULT_LOGGER_CHANNEL_NAME)
    {
        ...
        return \trailingslashit(\untrailingslashit($upload_dir['basedir'])) . \ILKinguinVendor\WPDesk\Logger\WP\WPCapture::LOG_DIR . \DIRECTORY_SEPARATOR . $name . '_debug.log';
    }
    /**
     ...
     * @return void
     */
    private function removeAllHandlers(\ILKinguinVendor\Monolog\Logger $logger)
    {
        try {
            while (\true) {
                $logger->popHandler();
            ...
            $logger->pushHandler(new \ILKinguinVendor\Monolog\Handler\NullHandler());
        }
    }
    /**
    ...
    {
        return \ILKinguinVendor\Monolog\Registry::hasLogger($name);
    }
}
