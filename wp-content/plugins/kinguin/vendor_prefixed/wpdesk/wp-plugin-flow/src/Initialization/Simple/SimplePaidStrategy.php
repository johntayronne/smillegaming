...
use ILKinguinVendor\WPDesk\Helper\HelperRemover;
use ILKinguinVendor\WPDesk\Helper\PrefixedHelperAsLibrary;
use ILKinguinVendor\WPDesk\License\PluginRegistrator;
use ILKinguinVendor\WPDesk\Plugin\Flow\Initialization\ActivationTrait;
use ILKinguinVendor\WPDesk\Plugin\Flow\Initialization\BuilderTrait;
use ILKinguinVendor\WPDesk\Plugin\Flow\Initialization\PluginDisablerByFileTrait;
use ILKinguinVendor\WPDesk\Plugin\Flow\Initialization\InitializationStrategy;
use ILKinguinVendor\WPDesk\PluginBuilder\Plugin\ActivationAware;
use ILKinguinVendor\WPDesk\PluginBuilder\Plugin\SlimPlugin;
/**
 ...
 */
class SimplePaidStrategy implements \ILKinguinVendor\WPDesk\Plugin\Flow\Initialization\InitializationStrategy
{
    ...
    /** @var SlimPlugin */
    private $plugin;
    public function __construct(\ILKinguinVendor\WPDesk_Plugin_Info $plugin_info)
    {
     ...
     * @return SlimPlugin
     */
    public function run_before_init(\ILKinguinVendor\WPDesk_Plugin_Info $plugin_info)
    {
     ...
     * @return SlimPlugin
     */
    public function run_init(\ILKinguinVendor\WPDesk_Plugin_Info $plugin_info)
    {
        if (!$this->plugin) {
            $this->plugin = $this->build_plugin($plugin_info);
        }
        ...
        \add_action('plugins_loaded', function () use($registrator) {
            $this->init_helper();
            $is_plugin_subscription_active = $registrator instanceof \ILKinguinVendor\WPDesk\License\PluginRegistrator && $registrator->is_active();
            if ($this->plugin instanceof \ILKinguinVendor\WPDesk\PluginBuilder\Plugin\ActivationAware && $is_plugin_subscription_active) {
    ...
    private function register_plugin()
    {
        if (\apply_filters('wpdesk_can_register_plugin', \true, $this->plugin_info)) {
            $registrator = new \ILKinguinVendor\WPDesk\License\PluginRegistrator($this->plugin_info);
    ...
    private function prevent_older_helpers()
    {
        ...
        if (\apply_filters('wpdesk_can_remove_old_helper_hooks', \true, $this->plugin_info)) {
            (new \ILKinguinVendor\WPDesk\Helper\HelperRemover())->hooks();
        }
    }
    /**
    ...
    {
        (new \ILKinguinVendor\WPDesk\Plugin\Flow\Initialization\PluginDisablerByFileTrait('wpdesk-helper/wpdesk-helper.php'))->disable();
    }
}
