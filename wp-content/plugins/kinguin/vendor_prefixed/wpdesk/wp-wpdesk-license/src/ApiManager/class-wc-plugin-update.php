 ...
 *
 */
if (!\class_exists('ILKinguinVendor\\WPDesk_Update_API_Check')) {
    class WPDesk_Update_API_Check
    {
        ...
        public function plugin_information($args)
        {
            ...
            if (\is_wp_error($request) || \wp_remote_retrieve_response_code($request) != 200) {
                if (\class_exists('ILKinguinVendor\\WPDesk_Logger_Factory')) {
                    if (\is_wp_error($request)) {
                        \ILKinguinVendor\WPDesk_Logger_Factory::log_wp_error($request, \debug_backtrace(\DEBUG_BACKTRACE_PROVIDE_OBJECT | \DEBUG_BACKTRACE_IGNORE_ARGS));
                    } else {
                        \ILKinguinVendor\WPDesk_Logger_Factory::log_message_backtrace('Response is invalid. Response: ' . \json_encode($request), \debug_backtrace(\DEBUG_BACKTRACE_PROVIDE_OBJECT | \DEBUG_BACKTRACE_IGNORE_ARGS), \ILKinguinVendor\WPDesk_Logger::ERROR );
                    }
                }
                return \false;
            }
            ...
            if (\is_object($response)) {
                ...
                if (\class_exists('ILKinguinVendor\\WPDesk_Logger_Factory')) {
                    \ILKinguinVendor\WPDesk_Logger_Factory::log_message_backtrace('Response is not an object', \debug_backtrace(\DEBUG_BACKTRACE_PROVIDE_OBJECT | \DEBUG_BACKTRACE_IGNORE_ARGS), \ILKinguinVendor\WPDesk_Logger::DEBUG );
                } else {
