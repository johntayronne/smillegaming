...
class WPDesk_Helper_List_Table extends \WP_List_Table
{
    ...
    {
        echo \wpautop(\__('No WP Desk plugins found.', 'kinguin'));
    }
    ...
    {
        $columns = ['product_name' => \__('Plugin', 'kinguin'), 'product_status' => \__('Subscription Status', 'kinguin'), 'product_license' => \__('Subscription Data', 'kinguin')];
        return $columns;
    }
    ...
    {
        $status = \__('Deactivated', 'kinguin');
        if ($item['activation_status'] == 'Activated') {
            $status = \__('Activated', 'kinguin');
        }
        return $status;
    }
