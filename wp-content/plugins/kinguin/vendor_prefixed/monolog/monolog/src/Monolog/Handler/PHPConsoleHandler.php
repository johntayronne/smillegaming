...
use ILKinguinVendor\Monolog\Utils;
use ILKinguinVendor\PhpConsole\Connector;
use ILKinguinVendor\PhpConsole\Handler;
use ILKinguinVendor\PhpConsole\Helper;
/**
 ...
 */
class PHPConsoleHandler extends \ILKinguinVendor\Monolog\Handler\AbstractProcessingHandler
{
     ...
     * @throws Exception
     */
    public function __construct(array $options = array(), \ILKinguinVendor\PhpConsole\Connector $connector = null, $level = \ILKinguinVendor\Monolog\Logger::DEBUG, $bubble = \true)
    {
        if (!\class_exists('ILKinguinVendor\\PhpConsole\\Connector')) {
    ...
    private function initConnector(\ILKinguinVendor\PhpConsole\Connector $connector = null)
    {
        if (!$connector) {
            if ($this->options['dataStorage']) {
                \ILKinguinVendor\PhpConsole\Connector::setPostponeStorage($this->options['dataStorage']);
            }
            $connector = \ILKinguinVendor\PhpConsole\Connector::getInstance();
        }
        if ($this->options['registerHelper'] && !\ILKinguinVendor\PhpConsole\Helper::isRegistered()) {
            \ILKinguinVendor\PhpConsole\Helper::register();
        }
        if ($this->options['enabled'] && $connector->isActiveClient()) {
            if ($this->options['useOwnErrorsHandler'] || $this->options['useOwnExceptionsHandler']) {
                $handler = \ILKinguinVendor\PhpConsole\Handler::getInstance();
    ...
    {
        if ($record['level'] < \ILKinguinVendor\Monolog\Logger::NOTICE) {
    ...
    private function handleDebugRecord(array $record)
    {
        ...
        if ($record['context']) {
            $message .= ' ' . \ILKinguinVendor\Monolog\Utils::jsonEncode($this->connector->getDumper()->dump(\array_filter($record['context'])), null, \true);
        }
    ...
    {
        return new \ILKinguinVendor\Monolog\Formatter\LineFormatter('%message%');
    }
}
